/* sw.js — service worker for {{APP_TITLE}}.
 *
 ...
 * shared origin:
 *   1. Never serve or cache anything outside /{{APP_ID}}/ (the SDK errors — and
 *      toasts in ?dev=1 — if launcher files land in a game cache).
 *   2. Never clean up origin-wide: delete only your own {{APP_ID}}-* caches.
 * Bump CACHE_NAME's suffix on every deploy so clients pick up new assets.
 */

const GAME_ID = '{{APP_ID}}';
